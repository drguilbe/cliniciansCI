...
#load(paste(datapath, "NCOMM_CIP_data.Rdata",sep=""))
all_data<-read.csv(paste(datapath, "NCOMM_CIP_data.csv",sep=""), stringsAsFactors = TRUE)
all_data$experimental_cond<-as.factor(all_data$experimental_cond)
all_data$patient_demo<-as.factor(all_data$patient_demo)
all_data$trial_id<-as.factor(all_data$trial_id)
all_data$round<-as.factor(all_data$round)
all_data$round <- factor(all_data$round, levels = c("Initial", "Second", "Final"))
...
all_data_chg<-all_data_agg %>% dplyr::group_by(experimental_cond, patient_demo, trial_id) %>% 
...
#Fig1AB Data#
#############
fig1AB_main<-all_data_agg %>% dplyr::group_by(experimental_cond, patient_demo, round) %>% 
...
fig1AB_inset<-all_data_chg %>% dplyr::group_by(experimental_cond, patient_demo) %>% 
...
##Figure 2B##
#############
Fig2B_data<-rbind(final_net_data, final_control_data)
...
Fig2B_data_final$Patient.Demographic<-Fig2B_data_final$patient_demo
Fig2B_data_final$Patient.Demographic<-as.factor(Fig2B_data_final$Patient.Demographic)
levels(Fig2B_data_final$Patient.Demographic)<-c("Black.Female","White.Male")
...
levels(Fig2B_data_final$Experimental.Condition)<-c("Control","Networks")
...
logit_main <- glm(Prob.Improve.Treatment.Accuracy ~ 
                    Improve.Diagnostic.Accuracy * Patient.Demographic * Experimental.Condition, 
...
all_effects_table<-all_effects_table$`Improve.Diagnostic.Accuracy:Patient.Demographic:Experimental.Condition`

ggplot(subset(all_effects_table, Experimental.Condition=="Networks"), 
       aes(x = Improve.Diagnostic.Accuracy, y = fit, group=Patient.Demographic, color=Patient.Demographic)) + 
...
Fig3_ineq_stat<-Fig3_agg_AC_diff %>% group_by(trial_id, round, experimental_cond) %>% 
...
#TableS9
Fig2B_data_final$Patient.Demographic<-Fig2B_data_final$patient_demo
Fig2B_data_final$Patient.Demographic = relevel(Fig2B_data_final$Patient.Demographic, ref=2)
...
all_data$Round.of.Recommendation.Num<-all_data$round_num
all_data$Patient.Demographic = relevel(all_data$Patient.Demographic, ref=2)
all_data$Experimental.Condition = relevel(all_data$Experimental.Condition, ref=2)

Table_s13a <- glm(
  formula = Unsafe.Undertreatment ~ Patient.Demographic + Round.of.Recommendation.Num * Experimental.Condition, 
  data=all_data, family = "binomial")
...
Table_s13b <- glm(
  formula = Undertreatment ~ Patient.Demographic + Round.of.Recommendation.Num * Experimental.Condition, 
  data=all_data, family = "binomial")
...
Table_s13d <- glm(
  formula = Overtreatment ~ Patient.Demographic + Round.of.Recommendation.Num * Experimental.Condition, 
  data=all_data, family = "binomial")
